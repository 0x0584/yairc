...
//   Updated: 2023/03/27 21:22:41 by archid           ###   ########.fr       //
/*                                                                            */
/* ************************************************************************** */
...
void server::accept_clients() {
	...
	while ((client_fd = ::accept(sock_fd_, addr_, &sock_len)) >= 0) {
		...
		map_users_.insert(std::pair<int, class user *>(client_fd, new class user(client_fd)));
...
user *server::user(int client_fd) const { return map_users_.at(client_fd); }
